...
resolve_bias_params <- function() {
  ...
  script_dir <- file.path(proj_dir, "code/03-chrombpnet/01-train_models")
  config_path <- file.path(script_dir, "../config.sh")
...
bias_model  <- resolve_bias_params()
...
count_unique_barcodes <- function(path) {
  as.numeric(
    str_split(
      trimws(
        system2(
          "bash",
          c("-lc", sprintf("cut -f4 %s | sort -u | wc -l", shQuote(path))),
          stdout = TRUE
        )
      ),
      "\\s+"
...
cluster_meta <- tibble(
  ...
  ncell = map_dbl(frag_files, count_unique_barcodes),
...
n_completed_models <- length(unique(all_metrics$Model))
n_completed_models <= 5 * n_clusters
...
cluster_meta <- cluster_meta %>% 
  mutate(
    Cluster_ChromBPNet = factor(Cluster_ChromBPNet, levels = sorted_cluster_order),
    ncell_plot = ifelse(is.finite(ncell) & ncell > 0, ncell, NA_real_)
  )
...
if (all(is.na(cluster_meta$ncell_plot))) {
  p2 <- ggplot() +
    annotate("text", x = 0, y = 0, label = "Cell counts unavailable for local-run inputs", size = 5) +
    xlim(-1, 1) + ylim(-1, 1) +
    theme_void() +
    ggtitle("Cells per cluster")
} else {
  p2 <- cluster_meta %>% 
    ggplot(aes(x = Cluster_ChromBPNet, y = ncell_plot)) +
    geom_bar(aes(fill = organ), stat = "identity") +
    scale_fill_manual(values = cmap_organ) +
    # put the scale on a log10 scale, but use non-logged numbers
    scale_y_log10(label = comma) +
